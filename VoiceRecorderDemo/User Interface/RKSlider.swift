...
@IBDesignable open class RKSlider: RKPropertyControl {
    ...
    @IBInspectable open var color: UIColor = RKStylist.sharedInstance.nextColor
    ...
    /// Slider style
    open var sliderStyle: RKSliderStyle = RKSliderStyle.tabIndicator
    ...
    public init(property: String,
                ...
                taper: Double = 1,
                format: String = "%0.3f",
                color: RKColor = RKStylist.sharedInstance.nextColor,
    ...
    private var indicatorWidth: CGFloat {
        ...
        case .tabIndicator: return RKSlider.tabIndicatorWidth
        }
    }

    var bgColorForTheme: RKColor {
        if let bgColor = bgColor { return bgColor }

        switch RKStylist.sharedInstance.theme {
        case .basic: return RKColor(white: 0.3, alpha: 1.0)
        case .midnight: return RKColor(white: 0.7, alpha: 1.0)
        }
    }

    var indicatorBorderColorForTheme: RKColor {
        if let indicatorBorderColor = indicatorBorderColor { return indicatorBorderColor }

        switch RKStylist.sharedInstance.theme {
        case .basic: return RKColor(white: 0.3, alpha: 1.0)
        case .midnight: return RKColor.white
        }
    }

    var sliderBorderColorForTheme: RKColor {
        if let sliderBorderColor = sliderBorderColor { return sliderBorderColor }

        switch RKStylist.sharedInstance.theme {
        case .basic: return RKColor(white: 0.2, alpha: 1.0)
        case .midnight: return RKColor(white: 0.9, alpha: 1.0)
        }
    }

    var textColorForTheme: RKColor {
    ...
    func drawFlatSlider(currentValue: CGFloat = 0,
        ...
        if showsValueBubble && isDragging {
            ...
            let bubbleSize = CGSize(width: valueLabelTextSize.width + RKSlider.bubblePadding.width,
                                    height: valueLabelTextSize.height + RKSlider.bubblePadding.height)
            ...
            let bubbleRect = CGRect(x: bubbleOriginX,
                                    y: sliderOrigin - valueLabelTextSize.height - RKSlider.bubbleMargin,
                                    width: bubbleSize.width,
                                    height: bubbleSize.height)
            let bubblePath = UIBezierPath(roundedRect: bubbleRect,
                                          byRoundingCorners: .allCorners,
                                          cornerRadii: CGSize(width: RKSlider.bubbleCornerRadius,
                                                              height: RKSlider.bubbleCornerRadius))
            color.setFill()
            bubblePath.fill()
            ...
            NSString(string: currentValueText).draw(
                in: CGRect(x: bubbleOriginX + ((bubbleSize.width - valueLabelTextSize.width) / 2.0),
                           y: sliderOrigin - valueLabelTextSize.height - RKSlider.bubbleMargin +
                            RKSlider.bubblePadding.height / 2.0,
