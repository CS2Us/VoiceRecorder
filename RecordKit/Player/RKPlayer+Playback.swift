...
extension RKPlayer {
    ...
    public func play(from startingTime: Double,
        ...
        // Note, final play command is in RKPlayer.swift for subclass override
    ...
    private func scheduleBuffer(at audioTime: AVAudioTime?) {
        ...
        // RKLog("Scheduling buffer...\(startTime) to \(endTime)")
        if #available(iOS 11, macOS 10.13, tvOS 11, *) {
    ...
    private func scheduleSegment(at audioTime: AVAudioTime?) {
        ...
        guard totalFrames > 0 else {
            RKLog("Unable to schedule file. totalFrames to play is \(totalFrames). audioFile.length is", audioFile.length)
            return
        }
    ...
    @objc internal func handleCallbackComplete(completionType: AVAudioPlayerNodeCompletionCallbackType) {
        // RKLog("\(audioFile?.url.lastPathComponent ?? "?") currentFrame:\(currentFrame) totalFrames:\(frameCount) currentTime:\(currentTime)/\(duration)")
        ...
        DispatchQueue.main.async {
            ...
            do {
                ...
                RKLog("Failed to check currentFrame and call completion handler: \(error)... Possible Media Service Reset?")
            }
        }
    }
    ...
    @objc private func handleComplete() {
        ...
        RKLog("Firing callback. currentFrame:", currentFrame, "frameCount:", frameCount)
