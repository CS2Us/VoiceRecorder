...
/**
 RKPlayer is meant to be a simple yet powerful audio player that just works. It supports
 ...
 ```
 guard let player = RKPlayer(url: url) else { return }
 player.completionHandler = { RKLog("Done") }
...
public class RKPlayer: RKNode {
    ...
    public struct Fade {
        ...
        public var inRampType: RKSettings.RampType = .exponential {
            willSet {
                if newValue != inRampType { needsUpdate = true }
            }
        }

        public var outRampType: RKSettings.RampType = .exponential {
            willSet {
        ...
        public var type: RKSettings.RampType = .exponential
    ...
    @objc public var faderNode: RKBooster?
    ...
    // these timers will go away when RecordKit is built for 10.13
    ...
    @objc public var completionHandler: RKCallback?
    ...
    @objc public var rampType: RKSettings.RampType = .exponential {
        didSet {
    ...
    @objc public convenience init?(url: URL) {
        ...
        do {
            ...
            RKLog("ERROR loading \(url.path) \(error)")
        }
        return nil
    }

    /// Create a player from an AVAudioFile (or RKAudioFile). If a file has previously
    /// been opened for writing, you can reset it to readOnly with the reopenFile flag.
    /// This is necessary in cases where RKMicrophone may of had access to the file.
    ...
    internal func initialize(restartIfPlaying: Bool = true) {
        ...
        if mixer.engine == nil {
            RecordKit.engine.attach(mixer)
        }

        if playerNode.engine == nil {
            RecordKit.engine.attach(playerNode)
        } else {
        ...
        if let faderNode = faderNode {
            if faderNode.avAudioUnitOrNode.engine == nil {
                RecordKit.engine.attach(faderNode.avAudioUnitOrNode)
            } else {
    ...
    internal func connectNodes() {
        ...
        if let faderNode = faderNode {
            RecordKit.connect(playerNode, to: faderNode.avAudioUnitOrNode, format: processingFormat)
            RecordKit.connect(faderNode.avAudioUnitOrNode, to: mixer, format: processingFormat)
        } else {
            RecordKit.connect(playerNode, to: mixer, format: processingFormat)
        }
    }
    ...
    public func play(from startingTime: Double, to endingTime: Double, at audioTime: AVAudioTime?, hostTime: UInt64?) {
        // RKLog(startingTime, "to", endingTime, "at", audioTime, "hostTime", hostTime)
    ...
    @objc public override func detach() {
        ...
        audioFile = nil
        buffer = nil
        RecordKit.detach(nodes: [mixer, playerNode])

        if let faderNode = faderNode {
            RecordKit.detach(nodes: [faderNode.avAudioUnitOrNode])
        }
        faderNode = nil
    }

    @objc deinit {
        RKLog("* deinit RKPlayer")
    }
}

// This used to be in a separate file but this broke setPosition

@objc extension RKPlayer: RKTiming {
