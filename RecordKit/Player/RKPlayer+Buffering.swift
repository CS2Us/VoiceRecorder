...
extension RKPlayer {
    ...
    internal func updateBuffer(force: Bool = false) {
        ...
        if !updateNeeded {
            // RKLog("No buffer update needed")
            return
        }

        guard audioFile.length > 0 else {
            RKLog("ERROR updateBuffer: Could not set PCM buffer -> " +
        ...
        guard framesToRead > 0 else {
            RKLog("Error, endFrame must be after startFrame. Unable to fill buffer.")
            return
        }
        ...
        do {
            ...
            RKLog("ERROR RKPlayer: Couldn't read data into buffer. \(err)")
            return
        }
        ...
        // RKLog("Filled buffer from \(startTime) to \(endTime)")
    }
