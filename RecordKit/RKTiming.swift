...
extension RKTiming {
    /**
     Starts an array of RKTimings at a position.
     ...
     */
    public static func syncStart(_ nodes: [RKTiming], at position: Double = 0) -> AVAudioTime {
        for node in nodes {
            node.stop()
        ...
        let bufferDuration = RKSettings.ioBufferDuration
        let referenceTime = RecordKit.engine.outputNode.lastRenderTime ?? AVAudioTime.now()
    ...
    /**
     ...
     - Parameter other: An already started RKTiming that position will be synchronized with.
     - Parameter audioTime: Future time in the audio render context that playback should begin.
     */
    public func synchronizeWith(other: RKTiming, at audioTime: AVAudioTime? = nil) {
        stop()
...
/// An RKTiming implementation that uses a node for it's render time info.
open class RKNodeTiming: NSObject {

    /// An output node used for tming info.
    open weak var node: RKOutput?
    ...
    public init(node: RKOutput) {
        self.node = node
    }

}

extension RKNodeTiming: RKTiming {
    ...
    open func start(at audioTime: AVAudioTime?) {
        ...
        baseTime = baseTime ?? lastRenderTime.offset(seconds: RKSettings.ioBufferDuration)
    }
