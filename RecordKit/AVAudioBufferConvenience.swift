...
extension AVAudioPCMBuffer {
    ...
    @discardableResult open func copy(from buffer: AVAudioPCMBuffer,
        ...
        if remainingCapacity == 0 {
            RKLog("AVAudioBuffer copy(from) - no capacity!")
            return 0
        }

        if format != buffer.format {
            RKLog("AVAudioBuffer copy(from) - formats must match!")
            return 0
        }
        ...
        if count <= 0 {
            RKLog("AVAudioBuffer copy(from) - No frames to copy!")
            return 0
        }
    ...
    open func normalize() -> AVAudioPCMBuffer? {
        ...
        guard let peak: AVAudioPCMBuffer.Peak = peak() else {
            RKLog("Failed getting peak amplitude, returning original buffer")
            return self
        }
        ...
        // RKLog("Old Peak", peakAmplitude, "New Peak", normalizedBuffer?.peak())
    ...
    open func fade(inTime: Double,
                   outTime: Double,
                   inRampType: RKSettings.RampType = .exponential,
                   outRampType: RKSettings.RampType = .exponential) -> AVAudioPCMBuffer? {

        guard let floatData = self.floatChannelData, inTime > 0 || outTime > 0 else {
            RKLog("Error fading buffer, returning original...")
            return self
        }
        ...
        // RKLog("rampType", rampType.rawValue, "fadeInPower", fadeInPower, "fadeOutPower", fadeOutPower)
