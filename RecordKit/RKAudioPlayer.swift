...
// AURE: ? @available(*, deprecated, renamed: "RKPlayer")
open class RKAudioPlayer: RKNode, RKToggleable {
    ...
    fileprivate var scheduledStopAction: RKScheduledAction?
    ...
    /// Will be triggered when RKAudioPlayer has finished to play.
    /// (will not as long as loop is on)
    @objc open dynamic var completionHandler: RKCallback?
    ...
    /// The current played RKAudioFile
    @objc open dynamic var audioFile: RKAudioFile {
        return internalAudioFile
    }

    /// Path to the currently loaded RKAudioFile
    ...
    @objc open dynamic var startTime: Double {
        ...
        set {
            ...
            if newValue > Double(endingFrame) / internalAudioFile.sampleRate && endingFrame > 0 {
                RKLog("ERROR: RKAudioPlayer cannot set a startTime bigger than the endTime: " +
                    "\(Double(endingFrame) / internalAudioFile.sampleRate) seconds")

            } else {
                startingFrame = UInt32(newValue * internalAudioFile.sampleRate)

                RKLog("RKAudioPlayer.startTime = \(newValue), startingFrame: \(startingFrame)")
    ...
    @objc open dynamic var endTime: Double {
        ...
        set {
            ...
            if newValue == internalEndTime {
                // RKLog("endTime is the same, so returning: \(newValue)")
                return
                ...
                RKLog("ERROR: RKAudioPlayer cannot set an endTime more than file's duration: \(duration) seconds or " +
                ...
                RKLog("RKAudioPlayer.endTime = \(newValue), endingFrame: \(endingFrame)")
    ...
    @objc open dynamic var scheduledTime: Double = 0 {
        didSet {
            ...
            scheduledAVTime = RKAudioPlayer.secondsToAVAudioTime(hostTime: hostTime, time: scheduledTime)
        }
    }
    ...
    ///   - completionHandler: RKCallback that will be triggered when the player end playing (useful for refreshing
    ///                        UI so we're not playing anymore, we stopped playing...)
    ///
    /// - Returns: an RKAudioPlayer if init succeeds, or nil if init fails. If fails, errors may be caught.
    ///
    @objc public init(file: RKAudioFile,
                      looping: Bool = false,
                      lazyBuffering: Bool = false,
                      completionHandler: RKCallback? = nil) throws {

        let readFile: RKAudioFile

        // Open the file for reading to avoid a crash when setting frame position
        // if the file was instantiated for writing
        do {
            readFile = try RKAudioFile(forReading: file.url)

        } catch let error as NSError {
            RKLog("RKAudioPlayer Error: cannot open file \(file.fileNamePlusExtension) for reading")
            RKLog("Error: \(error)")
            throw error
        }
        ...
        RecordKit.engine.attach(internalPlayer)
        RecordKit.engine.attach(internalMixer)
        ...
        RecordKit.engine.connect(internalPlayer, to: internalMixer, format: format)
    ...
    open func play(at when: AVAudioTime?) {
        ...
        if !playing {
            if audioFileBuffer != nil {
                ...
                RKLog("RKAudioPlayer Warning: cannot play an empty buffer")
            }
        } else {
            RKLog("RKAudioPlayer Warning: already playing")
        }
    }
    ...
    open func pause() {
        if playing {
            if !paused {
                ...
                RKLog("RKAudioPlayer Warning: already paused")
            }
        } else {
            RKLog("RKAudioPlayer Warning: Cannot pause when not playing")
        }
    }
    ...
    open func reloadFile() throws {
        ...
        var newAudioFile: RKAudioFile?

        do {
            newAudioFile = try RKAudioFile(forReading: internalAudioFile.url)
        } catch let error as NSError {
            RKLog("RKAudioPlayer Error: Couldn't reLoadFile")
            RKLog("Error: \(error)")
            throw error
        }
    ...
    /// Replace player's file with a new RKAudioFile file
    @objc open func replace(file: RKAudioFile) throws {
        internalAudioFile = file
        do {
            try reloadFile()
        } catch let error as NSError {
            RKLog("RKAudioPlayer Error: Couldn't reload replaced File: \"\(file.fileNamePlusExtension)\"")
            RKLog("Error: \(error)")
        }
        RKLog("RKAudioPlayer -> File with \"\(internalAudioFile.fileNamePlusExtension)\" Reloaded")
    }
    ...
    open func play(from startTime: Double, to endTime: Double, when scheduledTime: Double) {
        let hostTime = mach_absolute_time()
        let avTime = RKAudioPlayer.secondsToAVAudioTime(hostTime: hostTime, time: scheduledTime)
    ...
    ///              helper function RKAudioPlayer.secondToAVAudioTime(hostTime:time). hostTime is a call to
    ...
    open func play(from startTime: Double, to endTime: Double, avTime: AVAudioTime?) {
        ...
        if endingFrame > startingFrame {
            ...
            RKLog("ERROR RKaudioPlayer: cannot play, \(internalAudioFile.fileNamePlusExtension) " +
    ...
    open func getPeakTime() -> Double {
        ...
        return RKAudioFile.findPeak(pcmBuffer: buffer)
    }
    ...
    @objc open func stopAtNextLoopEnd() {
        ...
        scheduledStopAction = RKScheduledAction(interval: endTime - currentTime) {
            self.stop()
    ...
    fileprivate func updatePCMBuffer() {
        ...
        guard internalAudioFile.length > 0 else {
            RKLog("RKAudioPlayer Warning:  \"\(internalAudioFile.fileNamePlusExtension)\" is an empty file")
            return
        }
        ...
        if internalAudioFile.samplesCount > 0 {
            ...
            // RKLog("framesToPlayCount: \(framesToPlayCount) frameCapacity \(totalFrameCount)")
            ...
            do {
                ...
                RKLog("read \(audioFileBuffer?.frameLength ?? 0) frames into buffer")

            } catch {
                RKLog("ERROR RKaudioPlayer: Could not read data into buffer.")
                return
            }
            ...
            RKLog("ERROR updatePCMBuffer: Could not set PCM buffer -> " +
    ...
    fileprivate func reverseBuffer() {
        guard let buffer = audioFileBuffer else {
            RKLog("Unable to create buffer in reverseBuffer")
            return
        }
        ...
        // RKLog("reverse() preparing \(length) frames")
    ...
    fileprivate func fadeBuffer(inTime: Double = 0, outTime: Double = 0) {
        guard audioFileBuffer != nil else {
            RKLog("audioFileBuffer is nil")
            return
        }

        // do nothing in this case
        if inTime == 0 && outTime == 0 {
            RKLog("no fades specified.")
            return
        }
        ...
        RKLog("fadeBuffer() inTime: \(inTime) outTime: \(outTime)")
        ...
        // RKLog("fadeInPower \(fadeInPower) fadeOutPower \(fadeOutPower)")
    ...
    open override func detach() {
        RecordKit.detach(nodes: [self.avAudioNode])
        RecordKit.engine.detach(internalPlayer)
    }
}
